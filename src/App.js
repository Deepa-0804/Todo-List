...
export default function Todo() {
  ...
  const addTodo = () => {
    if (newTodo.trim() !== '') {
      ...
      setTodoList([...todoList, newTodoItem]);//This creates a new array that contains all the existing items from todoList followed by the new item newTodoItem.
      setNewTodo('');
    }
  };

  const deleteTodo = (id) => {
    const updatedTodoList = todoList.filter((todo) => todo.id !== id);// The todoList is filtered to remove the to-do item that matches the provided id.
